...
- **会话写锁：GUI 先到就采纳，不硬抢**。一条会话在宿主进程里只有一个 live agent，谁先 resume
  谁持有写句柄。你在 Web GUI 里打开 bot 的会话时，GUI 持有写句柄；插件这时**采纳**这个 live
  agent（和 GUI 共用同一个 agent/loop），而不是再 resume 一次撞上 `already owned`。所以
  bot 的会话可以「你开着看、它照常回」。采纳来的 agent 归 GUI 所有，插件停止时不会 dispose 它；
  GUI 关掉后，插件下一轮 roster 会自动改回自己持有写句柄。
- **能力说明书由聊天室下发**：插件只做回合 framing（标题、摘要、触发行、人格）。「怎么兑换
  token、怎么读附件、怎么建定时任务」这类聊天室 API 知识，由 lobby 在 roster 的 `bots[].manual`
  里渲染好，插件把其中的房间占位符 `{{roomId}}` 换成当前房间后**原样追加**——插件不认识任何
  聊天室端点，lobby 加能力也无需插件发版。
