...
/**
 * Render the block that teaches a bot how to read a room attachment.
 *
 * The bot holds no lobby session: the only thing it is given is its own exchange
 * secret, and the download route wants a personal token. So the prompt carries
 * the address plus the one command that trades the secret for a token, and never
 * the token itself — a prompt is recorded in the session log, a short-lived
 * exchange is not what leaks from there.
 * @param options - exchange inputs.
 * @param options.lobbyUrl - the lobby's base URL as this host reaches it.
 * @param options.botId - the bot's id.
 * @param options.secret - the bot's exchange secret.
 * @returns the prompt block, or `""` when the host does not know the address.
 */
export function exchangeBlock({ lobbyUrl, botId, secret }) {
  if (typeof lobbyUrl !== "string" || lobbyUrl.length === 0) return "";
  if (typeof secret !== "string" || secret.length === 0) return "";
  const base = lobbyUrl.replace(/\/+$/, "");
  return [
    "——",
    "【房间附件】读取房间里的文件前，先用你的兑换密钥换一个个人 token：",
    `  curl -s -X POST ${base}/api/bots/${botId}/token -H "Content-Type: application/json" -d '{"secret":"${secret}"}'`,
    "响应里的 token 放进 `Authorization: Bearer <token>`，再去请求附件 URL。",
  ].join("\n");
}
 ...
 * @param options.bot - the bot (`{id, nick, persona}`).
 ...
 * @param options.exchange - `{lobbyUrl, botId, secret}` when this bot may read attachments.
 * @param options.now - clock.
 * @returns the prompt text.
 */
export function formatPrompt({ bot, roomName, triggers, digest, includeDigest, exchange, now = new Date() }) {
  ...
  const exchange_ = exchange === undefined || exchange === null ? "" : exchangeBlock({ ...exchange, botId: exchange.botId ?? bot.id });
  if (exchange_.length > 0) lines.push(exchange_);
  return lines.join("\n");
}
