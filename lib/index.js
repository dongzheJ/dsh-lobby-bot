...
import { decideSession, SessionBook } from "./session-book.mjs";
...
class RoomDriver {
   ...
   */
  warm() {
    if (this.#handle !== undefined || this.#pendingSession !== undefined) return;
  ...
  onEvent({ event, data }) {
    ...
    if (event !== "message" && event !== "mention") return;
    ...
    if (!shouldTrigger({ bot: this.#bot, event, message: data, lastTurnAt: this.#lastPromptAt, botChain, now })) return;
  ...
  async #runTurn(triggers) {
    ...
    try {
      ...
      const prompt = formatPrompt({
        ...
        includeDigest,
        // The bot holds no lobby session, so it can only read a room attachment
        // by trading its own exchange secret for the owner's token. The roster
        // carries that secret; the address comes from this host's login.
        exchange: { lobbyUrl: this.#hub.baseUrl, botId: bot.id, secret: bot.exchangeSecret },
      });
