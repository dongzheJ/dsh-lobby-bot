...
class RoomDriver {
  ...
  onEvent({ event, data }) {
    ...
    if (verdict === "run") this.#spawnTurn([data]);
  ...
  /**
   * Start a turn without letting its failure escape.
   *
   * `#runTurn` is fire-and-forget: the only thing between an unexpected
   * rejection and a process-wide `unhandledRejection` (fatal in this host) is
   * this catch. `#runTurn` already handles its own known failures; this guards
   * the bookkeeping that runs before and after that handler.
   * @param triggers - the messages this turn answers.
   */
  #spawnTurn(triggers) {
    this.#runTurn(triggers).catch((error) => {
      this.#hub.logger.error(
        `lobby-bot: ${this.#bot.id}/${this.#room.id} 回合异常：${String(error?.message ?? error)}`,
      );
    });
  }
  ...
  async #runTurn(triggers) {
    ...
    try {
      // Bookkeeping is inside the try on purpose: `#runTurn` is invoked
      // fire-and-forget, so a rejection from this write would be an unhandled
      // rejection — which the harness treats as fatal and exits the whole host,
      // taking every bot offline. A failed write is a failed turn, nothing more.
      ...
      if (next !== undefined) this.#spawnTurn(next);
    }
  }
